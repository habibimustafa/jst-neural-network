...
.controller('process', function($scope){
	...
	$scope.kelas = {
		...
		perbarui: function(pola, yOut){
			var databaru = {bobot:{}, bias:null};
			for(var i=0; i<$scope.sets.inputs; i++) {
				databaru.bobot['w'+(i+1)] = $scope.sets.bobot['w'+(i+1)] + 
					( $scope.sets.alpha * ($scope.sets.table[pola].Target - yOut) * $scope.sets.table[pola]['x'+(i+1)] );
			}
	        databaru.bias = $scope.sets.bias + $scope.sets.alpha * ($scope.sets.table[pola].Target - yOut);
	        return databaru;
		},
			...
			do {
				...
				for(var pola=0; pola<$scope.sets.table.length; pola++){
					...
					if(yOut != $scope.sets.table[pola].Target){
						...
						var dbaru = this.perbarui(pola, yOut);
						epoch.pola.push({
							...
							perbarui: {
								status: true,
								bobot: dbaru.bobot,
								bias: ($scope.sets.biascek)?dbaru.bias:'-'
							}
						});
						$scope.sets.bobot = dbaru.bobot; 
						if($scope.sets.biascek) $scope.sets.bias  = dbaru.bias;
					} else {
	...
	$scope.test = {
		...
		run: function(){
			var sigmaIn = this.sigmaInput(this.inputs);
			if(!isNaN(sigmaIn)){
				let yIn  = $scope.sets.bias + sigmaIn;
				this.hasil = $scope.kelas.aktivasi(yIn);
			} else
				alert("Input x1 dan x2 belum diisi dengan benar!");
		}
	};
})